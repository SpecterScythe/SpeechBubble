...
namespace SpeechBubble
{
    public partial class SpeechBubbleFrm : Form
    {
        ...
        private void CheckKeyState(object sender, EventArgs e)
        {
            ...
            {
                HelpScreen form1 = Application.OpenForms.OfType<HelpScreen>().FirstOrDefault();
                if (form1 != null)
                {
                    form1.Activate();
                    form1.richTextBox11.Focus();//added for 1.0.3
                }
            }
            ...
            {
                if (TTs.State != SynthesizerState.Speaking)//removed an extra if statement which was the same thing
                {
            ...
            {
                ActiveControl = null;//say in the settings that this key deselects anything on the app windows
            }
            ...
            if (IsKeyDown(VK_OEM_2) && IsKeyDown(VK_CONTROL))
            {
                if (!QMarkPressed && NumOfQMarkPress < 2)
                {
                    ...
                    helpScreen.Show();
                    NumOfQMarkPress++;//removed a not needed qmark bool
                }
            }
        }
        [DllImport("User32.dll")]
            ...
            if (Properties.Settings.Default.SpeakAll == true && EmptyBubble == false)
            {
                ...
                try
                {
                    var payload = new
                    {
                        ...
                        speed = 1.0,//Fix this for 1.0.3
                        pitch = 1.0,//Fix this for 1.0.3
